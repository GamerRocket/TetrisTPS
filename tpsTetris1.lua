--TPS script made by Rocket (Dustin Priest) Version 1.1 (December 20th, 2019)
...
local function displayIndicators(tps)
	gui.drawtext(0, 8, string.format("Taps Per Second: %d\nTop TPS: %d", tps, topTPS));
	gui.drawtext(185, 186, string.format("Level %d\nTop TPS: %d",
				    current_level, current_levelTPS));
end;

local function displayExtras()
	if (tpsCounter > 0) then
		gui.drawtext(16, 49, string.format("Level TPS:%d", c_tps));
		tpsCounter = tpsCounter - 1;
	end;

	if (versionCounter > 0) then
		gui.drawtext(189, 8, "Version 1.1");
